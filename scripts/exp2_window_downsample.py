...
def main() -> None:
    ...
    logger.info(
        "Config: window-days=%s downsample=%s max-test=%s n-starts=%s seed=%s normalize=%s",
        args.window_days,
        args.downsample,
        args.max_test,
        args.n_starts,
    ...
    logger.info(
        "Transformed shapes: train=%s val=%s test=%s (T=%s)",
        X_train.shape,
        X_val.shape,
        X_test.shape,
        X_train.shape[1],
    ...
    logger.info("Effective grid: dt=%.4f t1=%.2f", dt_eff, t1_eff)
    ...
    title = f"{timestamp} — Exp2 (window={args.window_days}, downsample={args.downsample})"
    update_experiment_log(
        args.exp_log,
        exp_key="exp2",
        title=title,
        run_dir=out_dir,
        script="scripts/exp2_window_downsample.py",
