...
MSE optimizer per curve, and optionally trains neural models (MLP/CNN1D and
...
def main() -> None:
    ...
    if any(ml_flags):
        ...
        X_train_ml = normalize_series(splits["X_train"], method=args.normalize, population=pop_train)
        y_train = splits["y_train"]
        X_val_ml = normalize_series(splits["X_val"], method=args.normalize, population=pop_val)
        y_val = splits["y_val"]
        ...
        for name, enabled, builder in model_specs:
            ...
            y_pred = ml.predict_params(model, X_test_ml)
    ...
    baseline_methods = ["baseline_mse"] if args.run_baseline else []
    ...
    if args.save_predictions:
        ...
        save_predictions(
            ...
            metadata={
                "exp": "exp0",
                "scenario": "clean",
                "baseline_method": "baseline_mse" if args.run_baseline else None,
                "ml_architectures": sorted(y_pred_ml.keys()),
    ...
    if args.save_plots or args.save_plot_data:
        ...
        if use_fit_subset:
            ...
            y_pred_by_method = {"baseline_mse": y_pred} if args.run_baseline else {}
            for method_name, y_pred_full in y_pred_ml.items():
